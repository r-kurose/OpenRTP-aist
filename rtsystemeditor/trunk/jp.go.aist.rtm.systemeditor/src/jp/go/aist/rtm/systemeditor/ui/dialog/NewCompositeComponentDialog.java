...
public class NewCompositeComponentDialog extends TitleAreaDialog {
	...
	private List<AbstractComponent> baseComponents;
	...
	public NewCompositeComponentDialog(Shell parentShell,
			AbstractComponent compositeComponent,
			List<AbstractComponent> list,
			List<AbstractComponent> baseList) {
		super(parentShell);
	...
	public void notifyModified() {
		if (nameText.getText().equals("")
				|| pathCombo.getText().equals("") || !validateInput() ) {
	...
	private boolean validateInput() {
		for(AbstractComponent target : baseComponents) {
			if( nameText.getText().trim().equals(target.getInstanceNameL())) {
				this.setMessage("同じ名前のコンポーネントがすでに存在します", IMessageProvider.WARNING);
				return false;
			}
		}
		this.setMessage("");
